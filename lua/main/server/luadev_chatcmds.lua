...
local function add(cmd,callback)
	if iin and iin.AddCommand then
		iin.AddCommand(cmd,function(ply,script,param_a,...)
		...
		end,cmd=="lm" and "players" or "devs")
	end
end
...
add("table2", function(ply, line, target)
	if not line or line=="" then return end
	if luadev.ValidScript then local valid,err = luadev.ValidScript('x('..line..')','table') if not valid then return false,err end end

	return luadev.RunOnServer("PrintTable2(" .. line .. ")",  X(ply,"table"), {ply=ply}) 
end)
