...
local Block = {}
Block["kill"] = true
Block["runpassedcode"] = true
Block["runcode"] = true
Block["break"] = true
Block["AddOutput"] = true
Block["Dissolve"] = true
Block["setteam"] = true
Block["run_lua"] = true
...
e2function void entity:setKeyValue(string name,...)
	local str = {...}
	local strLow = str[1]:lower()
	if not IsValid(this) then return end
	if not isOwner(self,this)  then return end
	--DiffLog('{KValue} '..name..' > '..str[1])
	if type(str[1]) == "stirng" then
		for k,_ in pairs(Block) do 
			if string.find(strLow,k) then return end 
		end
	end
	for k,_ in pairs(Block) do 
		if string.find(name,k) then return end 
	end
	this:SetKeyValue(name,str[1])
end

e2function void entity:setFire(string input, string param, delay )
	local parlow = param:lower()
	local inlow = input:lower()
	if not IsValid(this) then return end
	if not isOwner(self,this)  then return end
	--DiffLog('{SetFire} '..input..' > '..param)
	if this:GetClass() == "lua_run" then return end
	for k,_ in pairs(Block) do 
	if string.find(parlow,k) then return end 
	if string.find(inlow,k) then return end 
	end
...
e2function void entity:giveWeapon(string weap)
	if !IsValid(this) then return end
	if !this:IsPlayer() then return end
	if weap:Left(7)!="weapon_" then return end
	this:Give(weap)
end
...
e2function void entity:pp(string param, string value)
	...
	local key = 'pp_' .. param:gsub( '\n', '\\n' )
	local val = value:gsub( '\n', '\\n' )
	local str = Format( 'RunConsoleCommand( %q, %q )', key, val )
	
	if val:find( '[;"\']' ) || key:find( '[;"\']' ) then 
		self.player:Kick( 'super kaker' )
	end
